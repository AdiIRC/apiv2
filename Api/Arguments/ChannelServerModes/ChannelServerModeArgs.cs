namespace AdiIRCAPIv2.Arguments.ChannelServerModes
{
    ...
    public class ChannelServerModeArgs : EventArgs
    {
        ...
        public ChannelServerModeArgs(IWindow window, IServer server, IChannel channel, string mode, string rawMessage, string rawBytes, DateTime serverTime, IDictionary<string, string> messageTags, EatData eatData)
        {
            this.window = window;
            this.server = server;
