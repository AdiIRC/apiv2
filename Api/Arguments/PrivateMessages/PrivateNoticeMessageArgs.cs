namespace AdiIRCAPIv2.Arguments.PrivateMessages
{
    ...
    public class PrivateNoticeMessageArgs : EventArgs
    {
        ...
        public PrivateNoticeMessageArgs(IWindow window, IServer server, IUser user, string message, string rawMessage, string rawBytes, DateTime serverTime, IDictionary<string, string> messageTags, EatData eatData)
        {
            this.window = window;
            this.server = server;
            this.user = user;
