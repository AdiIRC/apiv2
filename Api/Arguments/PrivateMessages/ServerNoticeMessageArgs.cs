namespace AdiIRCAPIv2.Arguments.PrivateMessages
{
    ...
    /// <summary>
    ///     Arguments class passed to the ServerNoticeMessage event
    /// </summary>
    public class ServerNoticeMessageArgs : EventArgs
    {
        ...
        public ServerNoticeMessageArgs(IWindow window, IServer server, IUser user, string message, string rawMessage, string rawBytes, DateTime serverTime, IDictionary<string, string> messageTags, EatData eatData)
        {
            this.window = window;
            this.server = server;
            this.user = user;
