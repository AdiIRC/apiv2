namespace AdiIRCAPIv2.Arguments.PrivateMessages
{
    ...
    public class ServerNoticeMessageArgs : EventArgs
    {
        ...
        public ServerNoticeMessageArgs(IServer server, IUser user, string message, string rawMessage, string rawBytes, DateTime serverTime, IDictionary<string, string> messageTags, EatData eatData)
        {
            this.server = server;
            this.user = user;
