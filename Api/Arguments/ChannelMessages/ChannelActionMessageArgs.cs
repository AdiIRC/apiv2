namespace AdiIRCAPIv2.Arguments.ChannelMessages
{
    ...
    public class ChannelActionMessageArgs : EventArgs
    {
        ...
        public ChannelActionMessageArgs(IWindow window, IServer server, IChannel channel, IChannelUser user, string message, string rawMessage, string rawBytes, DateTime serverTime, IDictionary<string, string> messageTags, EatData eatData)
        {
            this.window = window;
            this.server = server;
